            ...
            try:
                ...
                if user_input:
                    ...
                    if "exit" in user_input.lower():
                        ...
                        if story:
                            speak_text(story)
                    elif "math" in user_input.lower():  # Check if user wants to perform math operations
                        solve_math_expression(user_input)
                    else:
...
def solve_math_expression(user_input):
    # Extract the math expression using regular expressions
    math_expression = re.search(r'(?<=math\s).*', user_input, re.IGNORECASE).group().strip()
    try:
        result = eval(math_expression)
        speak_text(f"The result of {math_expression} is {result}")
    except Exception as e:
        print("Error evaluating math expression:", e)
        speak_text("Sorry, I couldn't evaluate the math expression.")
